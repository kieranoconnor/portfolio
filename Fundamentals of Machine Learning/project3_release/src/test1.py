...
def test_relu_activate():
    ...
    assert res.shape == (2, 3)
    print (res)
    assert np.allclose(res, np.array([[0.1, 0.,  0. ], [0.2, 0.,  6. ]]), atol=1e-6)

# -------------------------------------------------------------------------
def test_relu_gradient():
    ''' (5 points) ReLU:gradient()'''
    ...
    assert res.shape == (2, 3)
    print (res)
    assert np.allclose(res, np.array([[1., 0., 0.], [1., 0., 1.]]), atol=1e-6)
...
def test_crossentropyloss_loss():
    ...
    assert np.allclose(res, 0.80264854, atol=1e-6)
